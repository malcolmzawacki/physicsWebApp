...
def generate_sci_not_problem(difficulty):
    ...
    if difficulty == 'easy': # two input, no negatives
        ...
        if flip == 0: # 2 mul 1 div
            ...
            q_term_2, q_exp_2 = sci_not_format(q_term_2, q_exp_2)
            ans_term, ans_exp = sci_not_format(ans_term,ans_exp)
...
def new_question(difficulty):
    st.session_state.problem, st.session_state.ans1, st.session_state.ans2 = generate_sci_not_problem(difficulty)
    ...
    if st.session_state.problem:
        ...
        with col2:
            ...
            user_input1 = st.number_input(label='leading term', value=None,
                step=None,
                format="%f", key=f"user_input1{st.session_state.question_id}")
        with col3:
            st.header("")
        ...
        with col4:
            ...
            user_input2 = st.number_input(label='exponent', value=None,
                step=None,
                format="%f", key=f"user_input2{st.session_state.question_id}")
        with col1:
            st.write("")
        with col6:
            st.write("")
